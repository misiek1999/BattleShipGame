...
void PlayerBotBase::onRequestResult(const RequestId req_id, const GameSession::RequestResult result) {
    LOG_V("PlayerBotBase received request result: {} for request ID: {}", to_cstring(result), req_id);
...
void PlayerBotBase::onRoundEnded(const GameEngine::RoundResult round_result) {
    ...
    request_map_.clear();
    lock.unlock();
    // Bot player can start round immediatly
...
void PlayerBotBase::placeShipsHelper() {
    if (current_game_status_ != GameEngine::GameStatus::PreparingBoards) {
        LOG_I("Unable to place ship, when game is in state: {}", int(current_game_status_));
    }
    auto ship_type = Board::ShipType::Undefined;
    ...
    for (std::size_t i = 0; i < available_bot_ships_.size(); ++i) {
        if (available_bot_ships_[i] > 0) {
            ship_type = static_cast<Board::ShipType>(i);
            break;
        }
    }
    if (ship_type == Board::ShipType::Undefined) {
        LOG_I("All ships placed. Stop futher ship placing on board");
        const auto board_str = Board::board_to_ascii_string(this->board_);
        LOG_V("Computer board({}):\n{}",to_cstring(player_type_), board_str);
        return;
    }
...
void PlayerBotBase::makeShotHelper() {
    if (current_game_status_ != GameEngine::GameStatus::RoundInProgress) {
